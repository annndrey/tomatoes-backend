...
class CompClassifier():
    def __init__(self, **kwargs):
        sd_model_name = kwargs['sd_model_name']
        sd_model_path = kwargs['sd_model_path']
        self.sd_model = self._load_sd_model(sd_model_name, sd_model_path)
        cl_model_name = kwargs['cl_model_name']
        cl_model_path = kwargs['cl_model_path']
        cl_model_mode = kwargs['cl_model_mode']
        num_classes = kwargs['num_classes']
        self.cl_model = self._load_cl_model(cl_model_name, cl_model_path, cl_model_mode, num_classes)
        self.transform = self._get_square_transform()
        self.fnt = ImageFont.truetype('./FreeMono.ttf', 60)
        self.fnt_ = ImageFont.truetype('./FreeMonoBold.ttf', 10)
    ...
    def _load_cl_model(self, model_name, path_to_model, cl_model_mode, num_classes):
        """
        ...
        if cl_model_mode == 'new':
            self.idx_to_class = {idx:cls for idx,cls in enumerate(checkpoint['classes'])}
            model_inited.load_state_dict(loaded_state_dict)
        elif cl_model_mode == 'old':
            new_loaded_state_dict = {}
            for key in loaded_state_dict.keys():
                new_key = '.'.join(key.split('.')[1:])
                new_loaded_state_dict[new_key] = loaded_state_dict[key]
            self.idx_to_class = {idx:cls for cls,idx in checkpoint['class_to_idx'].items()}
            model_inited.load_state_dict(new_loaded_state_dict)
        model_inited.eval()
        return model_inited
    ...
    def _initialize_model(self, model_name, num_classes,
        ...
        if model_name == "resnet50":
            """ Resnet50
            """
            model_inited = models.resnet50(pretrained=use_pretrained)
            self.set_parameter_requires_grad(model_inited, feature_extract)
            num_ftrs = model_inited.fc.in_features
            model_inited.fc = nn.Linear(num_ftrs, num_classes)
    ...
    def parse_request_picture(self, img, max_n_of_leaves):
        ...
        n, m = self._get_init_grid()
        zones = self._get_zones(self.original, m, n)
        for z in zones.keys():
    ...
    def _get_init_grid(self):
        ...
        valid_w = [W/i for i in range(1,6) if W/i>224]
        valid_h = [H/i for i in range(1,6) if H/i>224]
        distances = []
        for i in range(len(valid_w)):
            for j in range(len(valid_h)):
                distances.append((i,j,np.abs(valid_w[i]-valid_h[j])))
        min_distance = sorted(distances, key=lambda x: x[-1])[0]
        n, m = min_distance[0], min_distance[1]
        return n, m
    ...
    def _cut_zones(self, max_n_of_leaves):
        ...
        for borders in self.final_borders:
            ...
            if zone.size[0] > 224 and zone.size[1]>224:
                zones.append(zone)
    ...
    def classify_final_zones(self):
        ...
        results = [self._process_result(res) for res in results]
        output_res = []
        ##print([self.final_borders, results])
        ...
        for borders,res in zip(self.final_borders,results):
            if 'State' in res:# and res['State'] == 'unhealthy':
                #print(["RES", res])
                output_res.append({"result": res, "region": borders})
        #        self.dr.rectangle(borders, outline = '#ff0000', width=3)
        #        self.dr.text((borders[0]+2, borders[1]+2), f"Object: {res['Object']}", font=self.fnt_)
        #        self.dr.text((borders[0]+2, borders[1]+12), f"State: {res['State']}", font=self.fnt_)
        #        self.dr.text((borders[0]+2, borders[1]+22), f"Diagnosis: {res['Diagnosis']}", font=self.fnt_)
        return output_res

    def _process_result(self, res):
        res = res.split('_')
        final_result = {}
        final_result['Object'] = res[0]
        if len(res) > 1:
            if res[1] == 'healthy':
                final_result['State'] = 'healthy'
            else:
                final_result['State'] = 'unhealthy'
                diagnosis = 'unknown' if res[1]=='unhealthy' else ' '.join(res[1:])
                final_result['Diagnosis'] = diagnosis
        return final_result


#sd_model_name = 'ResNet-50' # 'AlexNet' or 'DenseNet' r 'ResNet-50'
#sd_model_path = "/home/alex/Study/Work/ResNet-50_for_size_detection.pth"
#cl_model_name = 'resnet50'
#cl_model_path = "/home/alex/Desktop/Work/Size_detection/ResNet-50_for_classification.md"
#num_classes = 10
